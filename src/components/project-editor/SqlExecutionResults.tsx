...
export default function SqlExecutionResults({
    ...
    const isFinished = results.length > 0 && results.length === connections.length;
            ...
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    {hasFailures ? (
                        ...
                        <div className="size-3 rounded-full border-2 border-cerulean-400 border-t-transparent animate-spin" />
                    )}
                ...
                <div className="bg-ink-black-900 border border-cerulean-900/50 px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest text-cerulean-100">
                    <span className="text-green-400">{successful}</span> <span className="text-ink-black-400 mx-1">/</span> <span className="text-white">{connections.length || results.length}</span>
                </div>
            </div>
